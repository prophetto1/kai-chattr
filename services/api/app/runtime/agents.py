...
class AgentTrigger:
    ...
    def get_status(self) -> dict:
        from app.mcp.bridge import is_online, is_active, get_role
    ...
    def consume_queue(self, agent_name: str) -> list[dict]:
        """Atomically-ish drain queued triggers for a registered agent.

        Local wrappers read the queue file directly. Remote wrappers use this
        through the authenticated HTTP poll endpoint because their filesystem
        is not the server's filesystem.
        """
        ...
        try:
            raw_lines = queue_file.read_text("utf-8").splitlines()
