...
def capture_terminal(max_lines: int = 120) -> str:
    """Return the currently visible console text for browser-side inspection."""
    ...
    if max_lines and max_lines > 0:
        rect.Top = max(rect.Top, rect.Bottom - int(max_lines) + 1)
    ...
    for y in range(height):
        ...
        for x in range(width):
            ch = char_info_array[y * width + x].Char.UnicodeChar
