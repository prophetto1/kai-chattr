...
def capture_terminal(session_name: str, max_lines: int = 120) -> str:
    """Return the current tmux pane text for browser-side inspection."""
    try:
        start = f"-{int(max_lines)}" if max_lines and max_lines > 0 else "-"
        result = subprocess.run(
...
def run_agent(
    ...
    session_name = session_name or f"chattr-{agent}"
    agent_cmd = " ".join(
