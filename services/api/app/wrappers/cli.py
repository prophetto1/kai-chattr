...
def _write_claude_mcp_config(
    ...
    *,
    ...
    # Add chattr with bearer token for direct server auth
...
def _get_server_url(mcp_cfg: dict, transport: str) -> str:
    ...
    if transport == "sse":
        override = os.environ.get("CHATTR_MCP_SSE_URL", "").strip()
        if not override:
            override = os.environ.get("AGENTCHATTR_MCP_SSE_URL", "").strip()
        if override:
            return override.rstrip("/")
    else:
        override = os.environ.get("CHATTR_MCP_URL", "").strip()
        if not override:
            override = os.environ.get("AGENTCHATTR_MCP_URL", "").strip()
        if override:
            return override.rstrip("/")
    if transport == "sse":
...
def _server_base_url(server_port: int) -> str:
    return (
        os.environ.get("CHATTR_SERVER_URL", "").strip()
        or os.environ.get("AGENTCHATTR_SERVER_URL", "").strip()
        or f"http://127.0.0.1:{server_port}"
    ).rstrip("/")


def _api_url(server_port: int, path: str) -> str:
    return f"{_server_base_url(server_port)}{path}"


def _split_endpoint(url: str, default_path: str) -> tuple[str, str]:
    """Return (scheme://host:port, /path) for a remote MCP endpoint."""
...
def _register_instance(server_port: int, base: str, label: str | None = None) -> dict:
    ...
    if remote_token:
        headers["X-Chattr-Remote-Token"] = remote_token
        headers["X-Agentchattr-Remote-Token"] = remote_token
...
def _poll_remote_queue(server_port: int, agent_name: str, token: str) -> list[dict]:
    """Fetch and drain queued triggers from a remote chattr server."""
    if not token:
        return []
    try:
        import urllib.parse
        ...
        req = urllib.request.Request(
            _api_url(server_port, f"/api/poll/{safe_name}"),
...
def _post_terminal_snapshot(server_port: int, agent_name: str, token: str, text: str) -> bool:
    """Send the wrapper terminal's visible text to the server."""
    if not token:
        return False
    try:
        import urllib.parse
        ...
        safe_name = urllib.parse.quote(agent_name, safe="")
        body = json.dumps({"text": text, "captured_at": time.time()}).encode()
        req = urllib.request.Request(
            _api_url(server_port, f"/api/terminal/{safe_name}"),
            method="POST",
            data=body,
...
def _queue_watcher(get_identity_fn, inject_fn, *, is_multi_instance: bool = False, trigger_flag=None,
    ...
    while True:
        try:
            ...
            elif queue_file.exists() and queue_file.stat().st_size > 0:
...
def main():
    ...
    # Apply CHATTR_* overrides (from CLI flags or env; AGENTCHATTR_* fallback) BEFORE loading
    ...
    if args.server_url:
        ...
        os.environ["CHATTR_SERVER_URL"] = server_url
        os.environ.setdefault("AGENTCHATTR_SERVER_URL", server_url)
    if args.mcp_url:
        mcp_url = args.mcp_url.rstrip("/")
        os.environ["CHATTR_MCP_URL"] = mcp_url
        os.environ.setdefault("AGENTCHATTR_MCP_URL", mcp_url)
    if args.mcp_sse_url:
        mcp_sse_url = args.mcp_sse_url.rstrip("/")
        os.environ["CHATTR_MCP_SSE_URL"] = mcp_sse_url
        os.environ.setdefault("AGENTCHATTR_MCP_SSE_URL", mcp_sse_url)
    if args.remote_agent_token:
        remote_token = args.remote_agent_token.strip()
        os.environ["CHATTR_REMOTE_AGENT_TOKEN"] = remote_token
        os.environ.setdefault("AGENTCHATTR_REMOTE_AGENT_TOKEN", remote_token)
    ...
    if needs_proxy:
        from app.mcp.proxy import McpIdentityProxy

        transport = inject_cfg.get("mcp_transport", "http")
        if transport == "sse":
            endpoint_override = (
                os.environ.get("CHATTR_MCP_SSE_URL", "").strip()
                or os.environ.get("AGENTCHATTR_MCP_SSE_URL", "").strip()
            )
        else:
            endpoint_override = (
                os.environ.get("CHATTR_MCP_URL", "").strip()
                or os.environ.get("AGENTCHATTR_MCP_URL", "").strip()
            )
    ...
    def _set_terminal_capture(capture_fn):
        nonlocal _terminal_capture
        _terminal_capture = capture_fn
    ...
    def _terminal_monitor():
        ...
        while True:
            time.sleep(2)
            if not _terminal_capture:
                continue
            try:
                text = _terminal_capture()
                if not isinstance(text, str):
                    continue
                current_name, _ = get_identity()
                current_token = get_token()
                now = time.time()
                changed = text != last_text or current_name != last_agent
                periodic = now - last_report_time >= 15
                if changed or periodic:
                    if _post_terminal_snapshot(server_port, current_name, current_token, text):
                        last_text = text
                        last_agent = current_name
                        last_report_time = now
            except Exception:
                pass
    ...
    if transport == "pty":
        ...
        from app.wrappers.unix import capture_terminal, get_activity_checker, run_agent

        unix_session_name = f"chattr-{assigned_name}"
        _set_activity_checker(get_activity_checker(unix_session_name, trigger_flag=_trigger_flag))
        _set_terminal_capture(lambda: capture_terminal(unix_session_name, max_lines=120))
