        ...
        <header className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-6 py-3">
          <div className="flex min-w-0 flex-1 items-center gap-3">
            <div className="min-w-[120px]">
              <h1 className="truncate text-[13px] font-semibold leading-tight">My Agents</h1>
          ...
          <div className="flex shrink-0 items-center gap-3">
            <Tabs onValueChange={(value) => setFilter(value as RosterFilter)} value={filter}>
              <TabsList className="h-8">
                {FILTERS.map((entry) => (
                  <TabsTrigger className="px-2.5 text-[11.5px]" key={entry.id} value={entry.id}>
                    {entry.label}
                  </TabsTrigger>
                ))}
              </TabsList>
            ...
            <Button
              className="h-8 gap-1.5 rounded-[5px] text-[12px]"
        ...
        <ScrollArea className="min-h-0 flex-1" viewportClassName="min-h-0">
          <div className="mx-auto grid w-full max-w-[1100px] gap-4 px-6 py-6 sm:grid-cols-2 xl:grid-cols-3">
            {agents.map((agent) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="line-clamp-2 text-[11.5px] leading-5 text-muted-foreground">
            ...
            >
              ...
              <span className="text-[12px] font-medium">Define agent</span>
              <span className="text-[10.5px] text-muted-foreground/70">Next free: Slot 4 of 20</span>
            </button>
          </div>
