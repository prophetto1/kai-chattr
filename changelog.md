...
- Two PM ladder calls made mid-migration (no new roles): compact diff/stat counters → existing `code.stat` (its stated purpose); 13px workbench chat text → `ui.body` (visual-preserving) rather than the ladder's 16px `prose.body`, leaving the 16px bump as a deliberate follow-up.
