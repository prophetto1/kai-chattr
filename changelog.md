...
### feat(web,api): JSON typography role overrides in Appearance
- Added persisted `type_overrides.roles` settings support so typography roles can override family, size, weight, and line-height through the settings API without changing the built-in default theme JSON.
- Changed `AppThemeProvider` to merge saved role overrides over `apps/web/src/config/themes/default.theme.json` before applying runtime `--type-*` CSS variables. Typography persistence is now explicit: the Appearance route drafts font-family and role edits locally, previews them through CSS variables, and sends one settings PATCH only when Save is pressed.
- Updated Settings -> Appearance with a grouped role editor using the existing shadcn/Radix `Accordion` primitive: Display opens by default; Interface, Reading, and Code can be opened/closed independently. Each role category has Apply-to-all steppers for size, weight, and line-height; role rows keep per-role Typeface selectors for actual face overrides plus size/weight/line exception steppers and reset. The bad raw size/weight/line text inputs were removed.
- Verification: `uv run pytest -q tests/test_theme_settings_api.py` exit 0 (8 passed); `pnpm --dir apps/web run build` exit 0; `git diff --check` exit 0 with only CRLF warnings; Playwright on `http://127.0.0.1:8800/settings/user/appearance` confirmed Save/Discard controls, Apply-to-all steppers, Code roles/no Mono roles, no visible raw inputs, role-level Typeface selectors with concrete options (`Inter`, `Geist`, `Space Grotesk`, `JetBrains Mono`, `IBM Plex Mono`, `Source Serif`, `Lora`), `display.title` draft preview from `Default (Space Grotesk)` to `Geist`, zero PATCH requests before Save or Discard, Discard restoring the persisted value, and no console errors.

### feat(web): add default JSON theme + runtime typography variables
- Added `apps/web/src/config/themes/default.theme.json` and `apps/web/src/config/theme.schema.json` so the built-in default theme is visible as JSON, including every typography role's family, size, line-height, weight, tracking, numeric, row-height, and selected-row weight defaults.
- Changed `apps/web/src/lib/design-system.ts` so `typographyStyle()` and `rowTypographyStyle()` resolve through per-role `--type-*` CSS variables with JSON-derived fallbacks, and added helpers to emit/apply those variables from a theme definition. `getTypographyRole()` now preserves literal role types, fixing the stale `selectedFontWeight` type-widening problem for `workbench.fileRow`.
- Changed `apps/web/src/components/theme/AppThemeProvider.tsx` to apply the built-in default theme's typography variables at runtime before existing per-slot font-family overrides, preserving the current font picker behavior while making role sizes/weights/families live-editable by CSS variable.
- Verification: `pnpm --dir apps/web run build` exit 0; `git diff --check` exit 0 with only CRLF warnings; Playwright proof on `http://127.0.0.1:8800/settings/user/appearance` changed the real `Settings` heading from `13px` to `21px` by setting `--type-ui-body-strong-size`. `pnpm --dir apps/web exec tsc --noEmit` still fails on existing unrelated issues in `conversation.tsx`, `chattr-api.ts`, `settings.tsx`, and `workbench.tsx`.
...
### feat(web): migrate surfaces onto typography roles (Phase 2 — COMPLETE)
Migrated ALL 154 hardcoded `text-[Npx]` literals across 21 files onto `typographyStyle(role)`, per-surface, each built + committed. Repo-wide grep now returns **zero** `text-[Npx]` in `apps/web/src`. Two ladder gaps were filled mid-migration: `ui.micro` (10px) for tiny badges and `ui.body-sm` (12px/400) for compact regular text / file-tree rows.
...
- `components/workbench/WorkbenchCompactRail.tsx` (7): nav/account labels → `ui.body-strong`; rail rows + section header → `ui.label`; uppercase group label → `ui.overline`; account secondary → `ui.caption`. Brand wordmark normalized 600→500 onto `ui.body-strong` (flag for a `ui.brand` role if a heavier wordmark is wanted).
- `routes/settings.tsx` (6): header titles + row label/value → `ui.body-strong`; descriptions → `ui.caption`; mono scope path → `code.inline`. Shared header `className` consts stripped of type utilities, role applied per usage site.
- `routes/observability.tsx` (14): header/row/metric titles + values → `ui.body-strong`; descriptions / loading / error / contract table → `ui.caption`; nav-group + panel eyebrows → `ui.overline`; status badge → `ui.micro`.
- `routes/product-section.tsx` (8): title / scope / route labels → `ui.body-strong`; description → `ui.caption`; route eyebrow → `ui.overline`; mono route badge → `code.stat`; mono route value → `code.inline`.
- Two PM ladder calls made mid-migration: compact diff/stat counters → existing `code.stat`; 13px workbench chat text → `ui.body` (visual-preserving) rather than the ladder's 16px `prose.body`, leaving the 16px bump as a deliberate follow-up.
- `routes/workbench.tsx` (19): 11px chrome/headers/empty/loading/error/tabs/save-button → `ui.caption`; mono diff rows + hunk headers → `code.diff`; diff stat counters → `code.stat`; status chip → `ui.overline`; chat `MessageContent`/`MessageResponse` + workbench shell root → `ui.body`.
- `routes/agent-detail.tsx` (24): page/metric titles (15px) → `ui.lg`; buttons (12px) → `ui.label`; 11.5px body/desc → `ui.caption`; 10/10.5px meta + tab labels + state badges → `ui.micro`; mono ids/timestamps → `code.stat`.
- `routes/agents-new.tsx` (17) + `routes/agents.tsx` (12): form Labels / step-nav / footer buttons → `ui.label`; titles/agent-names → `ui.body-strong`; descriptions/notes/warnings → `ui.caption`; step-circle + slot/meta → `ui.micro`; mono ids → `code.stat`.
- Lane C components — `AgentRuntimeOverlay` (10), `WorkspaceFileTree` (5), `AgentJoinMenu`/`AgentTerminalPane`/`DockWorkspace`/`launcher/AgentLauncherDialog` (1 each): 11px chrome/inputs/status → `ui.caption`; badges → `ui.micro`; runtime log `<pre>` + stat rows → `code.stat`; uppercase sidebar label → `ui.overline`.
- `ai-elements/file-tree.tsx` (4) + `routes/workspace-placeholders.tsx` (2): density root → `ui.caption`/`ui.body-sm`; status letter → `ui.overline`; actions stat → `code.stat`; field label → `ui.body-strong`, description → `ui.caption`. (This batch adds the `ui.body-sm` role.)
- Lane D board — `BoardDock` (8), `BoardRulesVisualStates` (3), `board/BoardItemRow` (2), `board/BoardSection` (2): card-title + tab triggers → `ui.body-strong`; counters/meta/descriptions → `ui.caption`; status/count badges → `ui.micro` / `code.stat`; uppercase section header → `ui.overline`.
- DONE: `pnpm --dir apps/web run build` exit 0; `grep -rE "text-\[[0-9]" apps/web/src` → 0 matches. Migration commits span `81533f8` → `098b8fe` on `dev`.
